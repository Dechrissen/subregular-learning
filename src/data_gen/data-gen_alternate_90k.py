...
# goes with /data_r/ and tags.txt or tags_remaining.txt
...
# It puts files of sizes 90k, 10k, and 1k strings into a data_r folder
...
def build (border, lang, lang_name, n):
    ...
    '''
    test3_files = ["data_r/90k/"+lang_name+"_Test3.txt",
                   "data_r/10k/"+lang_name+"_Test3.txt",
                   "data_r/1k/"+lang_name+"_Test3.txt"]
...
def by_len(ex, f, count):
    random_examples=pynini.randgen(ex,9000)
...
# function that will take the 90k word list and create 10k and 1k from that list
def prune(f, name):
    
    data = open(name).readlines()
    
    small = [open("data_r/10k/" + f, "w+"),
                open("data_r/1k/" + f, "w+")]
...
tags = open(path_to_fsa+"tags_remaining.txt")
tags = tags.readlines()
...
for x in tags:
    ...
    test1_pos_num = 4500
    test2_pos_num = 2250
    test3_pos_num = 4500 # not used
    
    ls_min_len = 31
    ls_max_len = 50

    dir_name = "data_r/90k/" + x
    ...
    # start with a file that has 90k words. from there, prune to have 10k and 1k from that file. 
    pos_dict, neg_dict = \
